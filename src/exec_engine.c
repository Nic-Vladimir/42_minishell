...
/*   Updated: 2025/03/31 01:50:43 by mgavorni         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	expand_home(t_env *env, char **expanded, const char *input)
{
	char	*home;
	...
	if (home)
	{
		*expanded = ft_strjoin_free(*expanded, home);
		if (input[1] == '/')
		{
			*expanded = ft_strjoin_free(*expanded, input + 1);
		}
	}
	else
	{
		*expanded = ft_strjoin_free(*expanded, input);
	}
}
char	*expand_var(t_env *env, const char *input)
{
	...
	if (!expanded)
		return (NULL);
	i = 0;
	if ((input[i] == '~' && input[i + 1] == '\0') || (input[i] == '~' && input[i
			+ 1] == '/'))
	{
		expand_home(env, &expanded, input);
	}
	else
	{
		while (input[i])
...
int	execute_logical_op(t_env *env, t_ast_node *node, int in_fd, int out_fd)
{
	...
	env->last_exit_code = left_status; // Simplified assignment
	if (node->type == NODE_AND)
	{
		if (left_status == 0) // Left succeeds
		{
			right_status = execute_node(env, node->right, in_fd, out_fd);
			env->last_exit_code = right_status;
			return (right_status);
		}
		// Left fails: if right is OR, evaluate it
		if (node->right && node->right->type == NODE_OR)
		{
			right_status = execute_logical_op(env, node->right, in_fd, out_fd);
			// env->last_exit_code already set in recursive call
			return (right_status);
		}
		return (left_status); // Left fails, no OR, stop
	}
	else if (node->type == NODE_OR)
	{
		if (left_status != 0) // Left fails
		{
			right_status = execute_node(env, node->right, in_fd, out_fd);
			env->last_exit_code = right_status;
			return (right_status);
		}
		return (left_status); // Left succeeds
	}
	fprintf(stderr, "Invalid logical operator node\n");
	return (1);
}
